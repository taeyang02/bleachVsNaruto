...
//		private var _connGameLogic:OptimisticClientLogic;
    ...
    /**
     * Join an online room by room code via relay server.
     */
    public function joinOnline(roomCode:String, back:Function):void {
        OnlineConfig.load();
        _joinBack = back;

        var relay:OnlineRelayClient = OnlineRelayClient.I;
        relay.onGameTcp  = onReceiveData;
        relay.onGameUdp  = udpDataHandler;
        relay.onPeerLeft = onOnlinePeerLeft;

        relay.connect(function ():void {
            relay.joinRoom(roomCode, LanGameModel.I.playerName, function (data:Object):void {
                var hv:HostVO = new HostVO();
                if (data.host) {
                    hv.ownerName = data.host.ownerName;
                    hv.name      = data.host.name;
                    hv.password  = data.host.password;
                    hv.gameMode  = data.host.gameMode;
                    hv.status    = data.host.status;
                }
                hv.roomCode = data.roomCode || roomCode;
                hv.ip       = OnlineConfig.host;
                hv.tcpPort  = OnlineConfig.port;
                hv.udpPort  = OnlineConfig.port;
                _host       = hv;

                // Same as LAN after TCP connect: send JOIN
    ...
    public function sendJoinIn():void {
        if (LANGameCtrl.I.isOnline) {
            OnlineRelayClient.I.sendGameTcpJson(SocketMsgFactory.createJoinInMsg());
            return;
        }
    ...
    public function sendChart(chart:String):void {
        if (LANGameCtrl.I.isOnline) {
            OnlineRelayClient.I.sendGameTcpJson(SocketMsgFactory.createChart(chart, LanGameModel.I.playerName));
            return;
        }
    ...
    public function sendTCP(o:Object):void {
        if (LANGameCtrl.I.isOnline) {
            OnlineRelayClient.I.sendGameTcp(o);
            return;
        }
        if (_socket) {
            _socket.send(o);
    ...
    public function sendUDP(o:Object):void {
        if (LANGameCtrl.I.isOnline) {
            OnlineRelayClient.I.sendGameUdp(o);
            return;
        }
        if (_udpSocket) {

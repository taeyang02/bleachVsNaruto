...
public class LANGameState implements IStage {
    ...
    private function onOnlineJoined():void {
        var room:LANRoomState = new LANRoomState();
        MainGame.stageCtrl.goStage(room);
        room.clientMode(LANClientCtrl.I.host);
    }
    ...
    private function btnHandler(e:SetBtnEvent):void {
        ...
        case 'JOIN GAME':
            if (LANGameCtrl.I.isOnline) {
                ...
                joinDlg.onClose               = onDialogClose;
                joinDlg.onJoined              = onOnlineJoined;
                MainGame.stageCtrl.addLayer(joinDlg, 0, 0);
            }
            else {
                showHostList();
            }
